...
def add_core_options(parser=None):
    ...
    parser.add_argument(
        '-work_dir', help='Path to the directory where the job will run (will be created if it doesn\'t exist)')
    return parser
...
def add_general_options(parser=None):
    ...
    parser.add_argument(
        '-blast_dir', help='Directory where ncbi blast is installed (binaries in expected in bin subdirectory)')
    parser.add_argument(
        '-classic_mode',
        metavar='True/False',
        help=
        'Preset options to run the original AMPLE clustering/truncation options (1 cluster, 3 subclustering radii, 3 sidechains)'
    )
    parser.add_argument(
        '-ccp4i2_xml', help='Path to CCP4I2 XML file - if not None indicates we are running under CCP4I2')
    parser.add_argument(
    ...
    parser.add_argument(
        '-early_terminate', metavar='True/False', help='Stop the run as soon as a success has been found.')
    ...
    parser.add_argument(
        '-LGA', metavar='path_to_LGA dir',
        help=argparse.SUPPRESS)  # help='pathway to LGA folder (not the exe) will use the \'lga\' executable. UNUSED')
    parser.add_argument(
        '-make_models',
        metavar='True/False',
        help=
        'run rosetta modeling, set to False to import pre-made models (required if making models locally default True)')
    ...
    parser.add_argument(
        '-mr_sequence', help="sequence file for crystal content (if different from what's given by -fasta)")
    ...
    parser.add_argument('-nr', metavar='nr', help='Path to the NR non-redundant sequence database')
    ...
    parser.add_argument('-spicker_exe', help='Path to spicker executable')
...
def add_contact_options(parser=None):
    ...
    contact_group.add_argument(
        '-disulfide_constraints_file', help='Disulfide residue constraints for ab initio modelling')
    contact_group.add_argument(
    ...
    contact_group.add_argument(
        '--no-contact-prediction', default=False, action="store_true", help="Do not predict contacts")
    contact_group.add_argument(
        '-restraints_factor',
        type=float,
...
def add_mr_options(parser=None):
    ...
    mr_group.add_argument(
        '-molrep_only', metavar='True/False', help='Only use Molrep for Molecular Replacement step in MRBUMP')
    ...
    mr_group.add_argument(
        '-phaser_only', metavar='True/False', help='Only use Phaser for Molecular Replacement step in MRBUMP')
    ...
    mr_group.add_argument(
        '-shelxe_rebuild', metavar='True/False', help='Rebuild shelxe traced pdb with buccaneer and arpwarp')
    mr_group.add_argument(
        '-shelxe_rebuild_arpwarp', metavar='True/False', help='Rebuild shelxe traced pdb with arpwarp')
    mr_group.add_argument(
        '-shelxe_rebuild_buccaneer', metavar='True/False', help='Rebuild shelxe traced pdb with buccaneer')
    mr_group.add_argument(
        '-use_scwrl', metavar='True/False', help='Remodel sidechains of the decoy models using Scwrl4')
...
def add_rosetta_options(parser=None):
    ...
    rosetta_group.add_argument(
        '-use_homs', metavar='True/False', help="Select ROSETTA fragments from homologous models")
    return parser
