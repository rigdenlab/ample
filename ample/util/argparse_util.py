...
class AmpleArgParser(object):
    ...
    def main(self, args=None, contacts=True, modelling=True, mol_rep=True):
        ...
        if mol_rep: self._add_MR(parser)
        if modelling: self._add_ROSETTA(parser)
        
        return parser.parse_args(args)

    def _add_CONTACTS(self, parser):
...
def process_command_line(args=None, contacts=True, modelling=True, mol_rep=True):
    ...
    """    
    args = AmpleArgParser().main(args, contacts=contacts, 
                                 modelling=modelling, mol_rep=mol_rep)
    return args
