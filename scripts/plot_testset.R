...
cls <- c(SHELXE_CC="numeric",
		...
		PHASER_TFZ="numeric",
		PHASER_LLG="numeric",
		num_placed_atoms="numeric",
		ensemble_num_atoms="numeric"
)
data <- read.table(file="/media/data/shared/testset/clusters10_full_allresults.csv",sep=',', header=T, stringsAsFactors=FALSE, na.strings="N/A", colClasses=cls)
...
data$num_placed_chains <- data$num_placed_atoms / data$ensemble_num_atoms
...
p <-ggplot(data=data, aes(x=num_residues, fill=factor(success) ) )
...
ggsave("ensembleNumResidues.png")

p <-ggplot(data=data, aes(x=ensemble_percent_model, fill=factor(success) ) )
#p + geom_histogram(alpha = 0.5, position = 'identity', binwidth = 5 ) +
p + geom_histogram( position = 'dodge', binwidth = 5 ) +
		scale_fill_manual( values=c(fcolour,scolour),
				name="Success/Failure",
...
ggsave("ensemblePercentModel.png")

# Number that solved under each side-chain treatment
p <-ggplot( data=data, aes( factor(side_chain_treatment), fill=factor(success) ) ) 
...
ggsave("sideChainTreatments.png")

# Number that solved under each sub-clustering radius
p <-ggplot( data=data, aes( factor(subcluster_radius_threshold), fill=factor(success) ) ) 
...
p <-ggplot(data=data, aes(SHELXE_CC, minRfre, colour=factor(successShelxe) ) )
...
p + geom_point() +
		...
		ylab("Min Rfree") +
		ggtitle("Shelxe Min Rfree score")
ggsave("minRfreeVsCC.eps")

# CC distribution
p <-ggplot(data=data, aes(x=SHELXE_CC, fill=factor(success) ) )
...
#coord_fixed() +
p <-ggplot(data=data[ data$success==1, ], aes(x=SXRBUCC_final_Rfree, y=SXRARP_final_Rfree) )
p + geom_point() +
...
## Dirty hack to get a legend
#data$howSolvedC <- scolour
#data[ data$pdbCode %in%  allFailed, ]$howSolvedC <- fcolour
#data[ data$pdbCode %in%  everythingRerun, ]$howSolvedC <- scolour
#data[ data$pdbCode %in%  rerunSolved, ]$howSolvedC <- pcolour
#data[ data$pdbCode %in%  manual, ]$howSolvedC <- pcolour
#data[ data$pdbCode %in%  onlySingle, ]$howSolvedC <- scolour
#
#data$howSolvedS <- 1
#data[ data$pdbCode %in%  allFailed, ]$howSolvedS <- -2
#data[ data$pdbCode %in%  everythingRerun, ]$howSolvedS <- 16
#data[ data$pdbCode %in%  rerunSolved, ]$howSolvedS <- 1
#data[ data$pdbCode %in%  manual, ]$howSolvedS <- 0
#data[ data$pdbCode %in%  onlySingle, ]$howSolvedS <- 13
#
#p <- ggplot()
#p + geom_point( data=data,
#				aes(x=resolution, y=fastaLength, shape=factor(howSolvedS), colour=howSolvedC ) ) +
...
# Put colour in the aes so that we get a legend
p <- ggplot()
